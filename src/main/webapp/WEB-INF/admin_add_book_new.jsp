								...
								<div class="form-group">
									<select id="authors" name="authors">
										<c:forEach var="author" items="${authors}">
											<option value="${author.getId()}">${author.getSurName()+=' '+=author.getName()}</option>
										</c:forEach>
									</select>
								</div>
								<div class="form-group">
									<input type="date" name="calendar" value=""> <input
										type="hidden" name="action" value="create_book">
								</div>
							</div>
						</div>
						<span class="input-group-btn"><input type="hidden"
							name="action" value="Add Book">
							<button type="submit" class="btn btn-primary btn-form display-4">Добавить книгу</button></span>
					</form>
				</div>
			</div>
		</div>
